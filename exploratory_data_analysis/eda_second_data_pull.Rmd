...
```{r}
skipgram_threshold <- 10000

skipgram_network <- skipgram_count %>%
  filter(n > skipgram_threshold) %>%
  graph_from_data_frame(directed = FALSE)
```
...
V(skipgram_network)$degree <- degree(graph = skipgram_network)

E(skipgram_network)$width <- E(skipgram_network)$n/max(E(skipgram_network)$n)

plot(
  bigram_network, 
  vertex.size = 0.3*V(skipgram_network)$degree,
  vertex.color = "lightblue",
  vertex.label.color = "black", 
  vertex.label.cex = 0.3, 
  vertex.label.dist = 0.2,
  edge.color = "gray", 
  edge.width = 1*E(skipgram_network)$width,
  main = "Bigram Count Network",
  alpha = 50, 
  layout = layout_with_fr(skipgram_network), 
)
